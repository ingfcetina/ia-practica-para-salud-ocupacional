# Ruta de aprendizaje

tags: #ruta #estudiantes #aprendizaje

Sigue esta ruta si estás usando el vault como estudiante o profesional que quiere aprender de forma progresiva.

## Ruta corta

1. [[00-empezar-aqui/bienvenida-estudiantes|Bienvenida para estudiantes]]
2. [[00-empezar-aqui/reglas-de-privacidad|Reglas de privacidad]]
3. [[01-conceptos-clave/que-es-programar-con-ia|Qué es programar con IA]]
4. [[00-empezar-aqui/instalacion-y-preparacion|Instalación y preparación]]
5. [[06-recursos/datasets-ejemplo|Datasets de ejemplo]]
6. [[04-herramientas-ia/chatgpt-web|ChatGPT web]]
7. [[02-practicas-guiadas/practica-01-analizar-ausentismo|Práctica 1: analizar ausentismo]]
8. [[03-python-desde-cero/leer-datos-csv|Leer datos CSV]]
9. [[assets/notebooks/README|Notebooks de ejemplo]]
10. [[05-mini-proyectos/checklist-inteligente|Checklist inteligente]]

## Ruta completa

| Paso | Qué haces | Nota |
|---:|---|---|
| 1 | Entender el método | Empieza por conceptos y privacidad. |
| 2 | Preparar herramientas | Instala Obsidian, Python y librerías siguiendo la guía. |
| 3 | Conocer los datasets | Revisa qué contiene cada CSV antes de analizar. |
| 4 | Practicar con IA en la web | Usa prompts seguros con datos sintéticos. |
| 5 | Leer datos con Python | Observa el código aunque todavía no domines la sintaxis. |
| 6 | Ejecutar notebooks | Usa celdas comentadas para aprender paso a paso. |
| 7 | Crear un prototipo | Prueba el checklist o una página interactiva. |
| 8 | Revisar críticamente | Separa hechos, hipótesis y decisiones profesionales. |

## Si algo falla

Si Python, una librería o una herramienta de IA no funciona, no te detengas. Lee el código, identifica qué intenta resolver y revisa la explicación. En este nivel, comprender el flujo de trabajo es más importante que memorizar comandos.

## Relacionado

- [[00-empezar-aqui/mapa-del-vault|Mapa del vault]]
- [[06-recursos/prompts|Banco de prompts]]
